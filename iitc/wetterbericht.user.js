// ==UserScript==
// @id             iitc-plugin-iwb-sbergmod@maruschka forked from iitc-plugin-wetterbericht@dazz
// @name           iitc: wetterbericht
// @version        0.1
// @namespace      https://github.com/jonatkins/ingress-intel-total-conversion
// @updateURL      https://github.com/kotori-web-solutions/iwb-sbergmod/raw/master/iitc/wetterbericht.user.js
// @downloadURL    https://github.com/kotori-web-solutions/iwb-sbergmod/raw/master/iitc/wetterbericht.user.js
...
function wrapper() {
  ...
  window.plugin.wetterbericht.setupCallback = function() {
    ...
    addHook('mapDataRefreshEnd', window.plugin.wetterbericht.portalDataLoaded);
  };

  window.plugin.wetterbericht.result = {};

  window.plugin.wetterbericht.portalDataLoaded = function() {

    var p = window.portals;
      ...
      $.each(citydata[area].portals, function(inde, pid) {
		...
		$.each(p, function(index, d) {
          if(index == pid) {
            ...
            var faction;
			if ( d.options.data.team == "R" ) { faction = "RESISTANCE"; };
			if ( d.options.data.team == "E" ) { faction = "ALIENS"; };
            ...
            if (typeof area_data[faction]['portals'][pid] === "undefined") {
				...
				area_data[faction]['sum']   += d.options.data.level;
	
				// area_data[faction]['maxAP'] += (window.getAttackApGain(portalDetail.get(pid), window.getPortalFieldsCount(pid), window.getPortalLinksCount(pid)));
	...
	window.plugin.wetterbericht.show();
  };
  ...
  window.plugin.wetterbericht.show = function() {
    ...
    $.each(window.plugin.wetterbericht.result, function(area, area_data) {
      ...
      $.each(area_data, function(faction, value) {
        ...
        s += '\t' + factions[faction] + '('+numP+'): ' + level + 'L';
	...
	$.ajax({
	  type: "POST",
	  url: "", // INSERT TARGET URL FOR AJAX POST HERE
	  data: { weather: forXml },
